...
import {
  Split, Users, Equal, Receipt, Plus, Minus, Trash2,
  CreditCard, Banknote, Smartphone, CheckCircle2, ArrowRight,
  PartyPopper, AlertTriangle, Calculator,
} from 'lucide-react'
...
interface SplitCheckDialogProps {
  ...
  taxTotal: number
  cartItems: CartItem[]
  onConfirmSplit: (parties: SplitParty[]) => void
  partySize?: number
  autoGratuityPercent?: number
  autoGratuityThreshold?: number
}

const EU_ALLERGEN_MAP: Record<number, string> = {
...
export function SplitCheckDialog({
  open,
  onClose,
  orderTotal,
  subtotal,
  ...
  const assignItemToParty = (itemId: string, partyId: string) => {
  ...
  const setPartyTip = (partyId: string, percent: number) => {
    setParties(prev => prev.map(p => {
  ...
  const togglePartyPayment = (partyId: string, method: 'cash' | 'card' | 'mobile') => {
    setParties(prev => prev.map(p =>
      p.id === partyId ? { ...p, paymentMethod: method } : p
    ))
  }

  const handleConfirmEqual = () => {
        ...
        <Tabs value={splitMode} onValueChange={(v) => setSplitMode(v as typeof splitMode)}>
          ...
          <TabsContent value="equal" className="space-y-4 mt-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => setEqualCount(Math.max(2, equalCount - 1))}>
                  <Minus className="h-3 w-3" />
                </Button>
                <span className="text-2xl font-bold w-12 text-center">{equalCount}</span>
                <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => setEqualCount(Math.min(20, equalCount + 1))}>
                ...
                <Card key={party.id} className="overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-2 bg-muted/50 border-b">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {parties.length > 1 && (
                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => removeParty(party.id)}>
                        ...
                        {cartItems.filter(item => party.items.includes(item.id)).map(item => (
                          <div key={item.id} className="flex items-center justify-between text-sm bg-muted/30 rounded px-2 py-1">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => unassignItem(item.id)}>
          ...
          <TabsContent value="custom" className="space-y-4 mt-4">
            ...
            {parties.map((party, idx) => (
              <div key={party.id} className="flex items-center gap-3">
                ...
                {parties.length > 1 && (
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => removeParty(party.id)}>
                    <Trash2 className="h-3 w-3 text-destructive" />
                  </Button>
                )}
              </div>
            ))}
            <Button variant="outline" onClick={addParty} size="sm" className="gap-1">
              <Plus className="h-3 w-3" /> Dodaj osebo
            </Button>
            <DialogFooter>
              <Button variant="outline" onClick={onClose}>Prekliči</Button>
              <Button onClick={() => { toast.info('Custom split — coming soon!') }} className="gap-1.5">
                <CheckCircle2 className="h-4 w-4" />
                Potrdi
              </Button>
            </DialogFooter>
