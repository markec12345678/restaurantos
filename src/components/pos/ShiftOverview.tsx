...
export function ShiftOverview() {
  ...
  const loadShiftData = async () => {
    try {
      ...
      // Naloži razpored
      const shiftRes = await fetch(`/api/staff-shifts?date=${today}`)
      ...
      const shiftEmployees: ShiftEmployee[] = (shiftData || []).map((shift: any) => {
        const emp = empData?.find?.((e: any) => e.id === shift.employeeId) || {}
        const timeEntry = timeData?.find?.((t: any) =>
  ...
  const handleBreak = async (employeeId: string, onBreak: boolean) => {
    try {
      await fetch('/api/time-entries', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
                ...
                <CardContent className="p-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="relative">
                      ...
                      <div className={`absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full border-2 border-background ${config.dotColor}`} />
                    </div>
