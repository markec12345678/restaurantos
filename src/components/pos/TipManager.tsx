...
import {
  DollarSign, Users, Clock, Star, HandCoins, CheckCircle2,
  ChevronLeft, ChevronRight, BarChart3, Equal, Scale,
  Edit, Award, TrendingUp, Wallet, CreditCard,
} from 'lucide-react'
...
const METHOD_LABELS: Record<string, { label: string; icon: any; desc: string }> = {
...
export function TipManager() {
  ...
  const { data: currentPool } = useQuery({
    queryKey: ['tip-pool', selectedDate],
    queryFn: async () => {
  ...
  const saveManualMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tip-pool', selectedDate] })
