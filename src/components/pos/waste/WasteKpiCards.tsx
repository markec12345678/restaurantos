...
import { WASTE_TARGET_PERCENT } from '@/lib/waste-reasons'
import type { WasteKpiCardsProps } from './constants'

// ============================================
// KPI KARTICE ODPADKOV (R119: vse metrike so REALNE iz ledgerja)
// ============================================
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <Card className={isOnTarget ? '' : 'border-red-200 dark:border-red-800'}>
        <CardContent className="p-3">
          ...
          {summary.reversedCount > 0 && (
            <p className="text-xs text-muted-foreground">{summary.reversedCount} razveljavljenih</p>
          )}
        </CardContent>
      </Card>
      <Card>
      ...
      <Card className={isOnTarget ? '' : 'border-amber-200 dark:border-amber-800'}>
        <CardContent className="p-3">
          <div className="flex items-center gap-2 mb-1">
            <Percent className="h-4 w-4 text-amber-500" aria-hidden="true" />
          ...
          <p className="text-xs text-muted-foreground">od COGS {fmtCurrency(summary.saleCogs)} · cilj: &le;{WASTE_TARGET_PERCENT}%</p>
        </CardContent>
      </Card>
      <Card>
        <CardContent className="p-3">
          <div className="flex items-center gap-2 mb-1">
            <TrendingDown className="h-4 w-4 text-purple-500" aria-hidden="true" />
            <span className="text-xs text-muted-foreground">Food Cost %</span>
          </div>
          <p className="text-xl font-bold">{summary.foodCostPercentage}%</p>
          <p className="text-xs text-muted-foreground">prihodek {fmtCurrency(summary.revenue)}</p>
        </CardContent>
      </Card>
    </div>
  )
