...
import {
  ShieldAlert, Filter, X, AlertTriangle, Info, Check,
...
export function AllergenBadge({ allergens, compact = false }: { allergens: string; compact?: boolean }) {
...
interface AllergenFilterBarProps {
  ...
  onExcludedChange: (allergens: string[]) => void
      ...
      <div className="flex items-center gap-2 px-4 py-2">
        ...
        >
          ...
          {activeFiltersCount > 0 && (
            <Badge className="bg-amber-500 text-white text-[9px] h-4 px-1 ml-1">
              {activeFiltersCount}
            </Badge>
          )}
            ...
            {excludedAllergens.map(code => {
              ...
              return (
                <Badge key={code} variant="outline" className="text-[9px] h-5 px-1.5 border-red-200 text-red-600 bg-red-50 dark:bg-red-900/20 gap-0.5 flex-shrink-0">
                  {info?.icon} {code}
                  <X className="h-2.5 w-2.5 cursor-pointer" onClick={() => toggleAllergen(code)} />
                </Badge>
              )
            })}
            {dietaryFilters.map(id => {
              ...
              return (
                <Badge key={id} variant="outline" className="text-[9px] h-5 px-1.5 border-green-200 text-green-600 bg-green-50 dark:bg-green-900/20 gap-0.5 flex-shrink-0">
                  {info?.icon} {info?.name}
                  <X className="h-2.5 w-2.5 cursor-pointer" onClick={() => toggleDietary(id)} />
                </Badge>
              )
            })}
          </div>
...
export function filterItemsByAllergens(
  ...
  return items.filter(item => {
    ...
    const codes = item[allergensField].split(',').map((s: string) => s.trim())
    return !codes.some(code => excludedAllergens.includes(code))
  })
}
