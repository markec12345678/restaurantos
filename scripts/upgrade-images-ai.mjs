 ...
 * Run this script when the AI API rate limit resets.
 * Uses z-ai-web-dev-sdk with automatic retry and rate limiting.
 * 
 * Usage: node scripts/upgrade-images-ai.mjs
 * 
 * It will process images one at a time with 5-second delays between requests.
 * If rate-limited, it waits 60 seconds and retries.
 * Already-generated AI images (files > 50KB) are skipped.
 */
...
const MIN_AI_SIZE = 50000; // AI-generated images are typically > 50KB, SVG-based are smaller
...
const RETRY_DELAY_MS = 60000; // 60 seconds on rate limit
const MAX_RETRIES = 3;
...
async function generateAIImage(zai, item, retryCount = 0) {
  try {
    ...
    if (err.message && err.message.includes('429') && retryCount < MAX_RETRIES) {
      console.log(`    ⏳ Rate limited, waiting ${RETRY_DELAY_MS/1000}s (retry ${retryCount + 1}/${MAX_RETRIES})...`);
      await sleep(RETRY_DELAY_MS);
      return generateAIImage(zai, item, retryCount + 1);
    }
...
async function main() {
  ...
  for (let i = 0; i < ITEMS.length; i++) {
    ...
    // Skip if already AI-generated (large file)
    ...
    const result = await generateAIImage(zai, item);
  ...
  console.log(`\n✅ Complete! Upgraded: ${upgraded}, Skipped (already AI): ${skipped}, Failed: ${failed}`);
}
