...
export default function QRMenuPage() {
  ...
  }, [cart.length, activeCategory]); // fetchUpsellSuggestions depends on cart items
  ...
  // Font size class
  const _fontClass = fontSize === 'xl' ? 'text-lg' : fontSize === 'large' ? 'text-base' : 'text-sm';
  const _headingClass = fontSize === 'xl' ? 'text-2xl' : fontSize === 'large' ? 'text-xl' : 'text-lg';
  const _priceClass = fontSize === 'xl' ? 'text-2xl' : fontSize === 'large' ? 'text-xl' : 'text-lg';
    ...
    <div className={`min-h-screen ${isDark ? 'bg-gray-950 text-gray-100' : isHighContrast ? 'bg-white text-black' : 'bg-gradient-to-b from-amber-50 via-orange-50/50 to-amber-100/80 text-gray-900'} ${fontSize === 'xl' ? 'text-lg' : fontSize === 'large' ? 'text-base' : ''}`}>
      {error && (
        <div className="mx-4 mt-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg flex items-center justify-between">
          ...
          <button onClick={() => setError('')} className="text-red-500 hover:text-red-700 text-lg font-bold ml-2">&times;</button>
        </div>
      )}
      ...
      {showAllergenInfo && (
        <div ref={allergenPanelRef} className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label="Informacije o alergenih" onKeyDown={(e) => { if (e.key === 'Escape') setShowAllergenInfo(false) }}>
      ...
      {/* ===== ITEM DETAIL MODAL ===== */}
      {showItemDetail && (
        <div ref={itemDetailRef} className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label={`Podrobnosti: ${showItemDetail.name}`} onKeyDown={(e) => { if (e.key === 'Escape') setShowItemDetail(null) }}>
          ...
          <div className={`absolute bottom-0 left-0 right-0 ${isDark ? 'bg-gray-900' : 'bg-white'} rounded-t-3xl shadow-2xl max-h-[85vh] overflow-auto`}>
            ...
            <div className="p-4 space-y-4">
              ...
              <button
                onClick={() => {
                  // FIX BUG-05: Preveri omejitve modifikatorjev pred dodajanjem
                  const validationError = validateModifierGroups()
                  if (validationError) {
                    alert(validationError)
                    return
                  }
                  addToCart(showItemDetail, selectedMods, itemNotes)
                }}
      ...
      {showCart && (
        <div ref={cartDrawerRef} className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label="Vaše naročilo" onKeyDown={(e) => { if (e.key === 'Escape') setShowCart(false) }}>
