...
export async function findPrinter(type: 'order' | 'receipt', printerId?: string): Promise<PrinterInfo | null> {
  ...
  if (printerId) {
    ...
    if (printer && printer.isActive && printer.ipAddress) {
      ...
      const customPort = rules.length > 0 ? rules[0].port : undefined
      return {
        id: printer.id,
        name: printer.name,
  ...
  for (const printer of printers) {
    // P1-9: Zod-validiran parser — neveljaven JSON v printRules preskoči
    // tiskalnik (namesto da bi sesul izbiro)
    const rules = parsePrintRules(printer.printRules)
