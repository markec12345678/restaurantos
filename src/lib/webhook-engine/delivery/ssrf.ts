...
export function isInternalUrl(url: string): boolean {
  try {
    ...
    // FIX: Node.js URL.hostname vrača IPv6 z oklepaji, npr. [::1] — odstrani jih
    ...
    // FIX HIGH: IPv4-mapped IPv6 loopback (::ffff:127.x.x.x ali ::ffff:7f00:1)
    if (/^::ffff:(127\.|7f00:)/.test(hostname)) return true
    // FIX HIGH: Link-local naslovi
    if (/^169\.254\./.test(hostname)) return true
    // FIX HIGH: IPv6 unique local (fc00::/7) — fc ali fd na začetku
    if (/^f[cd][0-9a-f]*:/.test(hostname) || hostname.startsWith('fc') || hostname.startsWith('fd')) return true
