...
import { Store, MapPin, Users, ShoppingBag, CheckCircle2, Phone, Activity } from 'lucide-react'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        {locs.map((loc) => (
          <Card
            ...
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setSelectedLocation(selectedLocation === loc.id ? null : loc.id) } }}
          >
            <CardContent className="p-4 space-y-3">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className={`h-10 w-10 rounded-lg flex items-center justify-center ${
                    ...
                    <Store className={`h-5 w-5 ${loc.isOpen ? 'text-green-600' : 'text-gray-500'}`} />
                  </div>
                  <div>
              ...
              {/* Status indikator */}
              <div className="flex items-center gap-2">
                <div className={`h-2 w-2 rounded-full ${loc.isActive ? 'bg-green-500' : 'bg-red-500'}`}><span className="sr-only">{loc.isActive ? 'Aktivna' : 'Neaktivna'}</span></div>
