...
export function RecipeScaling() {
  ...
  const loadRecipes = async () => {
    try {
      ...
      const mappedRecipes: Recipe[] = (recipesData || []).map((r: any) => {
        const ingredients: RecipeIngredient[] = (r.ingredients || r.items || []).map((ing: any) => {
          const invItem = invData?.find?.((i: any) => i.id === ing.inventoryItemId)
  ...
  const scaledTotalCost = scaledIngredients.reduce((s, i) => s + i.totalCost, 0)
  const scaledServings = selectedRecipe ? Math.round(originalServings * scaleFactor) : 0
  const scaledCostPerServing = scaledServings > 0 ? scaledTotalCost / scaledServings : 0
