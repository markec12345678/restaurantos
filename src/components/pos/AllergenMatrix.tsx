...
import { ShieldAlert, Search, AlertTriangle, CheckCircle2, ChevronDown, ChevronUp, Edit, Save, X, Info } from 'lucide-react'
...
export const AllergenMatrix = memo(function AllergenMatrix() {
  ...
  const updateAllergens = useMutation({
    mutationFn: async ({ itemId, allergens }: { itemId: string; allergens: string[] }) => {
      // FIX HIGH: Pošlji na pravilen endpoint — /api/menu-items/[id], ne /api/categories
      // FIX HIGH: Shrani alergene kot comma-separated CODES ("1,3,7"), ne JSON array IDs (["gluten","eggs","milk"])
      // AllergenFilter pričakuje comma-separated kode, AllergenMatrix je uporabljal JSON string IDs — neujemanje!
      // FIX ALLER-05 MEDIUM: Odstrani || id fallback — filtriraj neznane kode, ne shranjuj jih
