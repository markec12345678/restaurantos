...
import {
  Bell, BellRing, CheckCircle, Clock, ShoppingCart, Plus,
  UtensilsCrossed, Wallet, RefreshCw, Wifi, WifiOff,
  ArrowRight, ShoppingBag, ChevronRight, X, ChefHat,
  LogIn, HandMetal, Timer, Users, CreditCard, Eye
} from 'lucide-react'
...
// ─── PIN Login ─────────────────────────────────────────────────
function WaiterLogin({ onLogin }: { onLogin: (emp: { id: string; name: string; role: string }) => void }) {
...
export default function WaiterPage() {
  ...
  const { data: ordersData, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const statuses = ['FIRED', 'IN_PROGRESS', 'READY', 'OPEN']
      const results = await Promise.all(
        statuses.map(s => fetch(`/api/orders?status=${s}`, { headers }).then(r => r.json()))
      )
  ...
  const ordersWithReady = allOrders.filter(o =>
    o.items.some(i => i.status === 'READY') && o.status !== 'COMPLETED'
  )
  ...
  return (
    <div className="p-3 space-y-2">
      {orders.map(order => {
        ...
        const statusColor = order.status === 'READY' ? 'bg-emerald-500' : order.status === 'IN_PROGRESS' ? 'bg-blue-500' : order.status === 'FIRED' ? 'bg-orange-500' : 'bg-muted'
