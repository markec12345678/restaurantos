...
import {
  CheckCircle2, Clock, ChefHat, Flame, Package, Truck,
  PartyPopper, Phone, MapPin, UtensilsCrossed, Loader2,
...
// ─── Status koraki ─────────────────────────────────────────────
const STATUS_STEPS = [
  { key: 'received', label: 'Prejeto', labelEn: 'Received', icon: CheckCircle2, color: 'blue' },
  { key: 'in-progress', label: 'V pripravi', labelEn: 'Preparing', icon: ChefHat, color: 'amber' },
  { key: 'cooking', label: 'V peči', labelEn: 'In the oven', icon: Flame, color: 'orange' },
  { key: 'ready', label: 'Pripravljeno', labelEn: 'Ready', icon: Package, color: 'emerald' },
  { key: 'on-the-way', label: 'Na poti', labelEn: 'On the way', icon: Truck, color: 'purple' },
  { key: 'delivered', label: 'Dostavljeno', labelEn: 'Delivered', icon: PartyPopper, color: 'green' },
]
...
export default function OrderStatusPage() {
  ...
  const fetchOrder = useCallback(async () => {
    try {
      const res = await fetch(`/api/public/order-track?orderId=${orderId}`)
                    ...
                    <div className="flex flex-col items-center">
                      <div className={`flex items-center justify-center h-10 w-10 rounded-full transition-all duration-500 ${
                        isCompleted
                          ...
                          : isActive
                            ? `bg-${step.color}-500 text-white shadow-lg shadow-${step.color}-500/30 animate-pulse`
