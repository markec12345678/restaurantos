...
async function main() {
  ...
  const procurement = todayTx.filter(t => t.type === 'procurement').reduce((s, t) => s + t.totalCost, 0);
  const cogs = todayTx.filter(t => t.type === 'sale').reduce((s, t) => s + Math.abs(t.totalCost), 0);
  const writeOffs = todayTx.filter(t => t.type === 'write-off' || t.type === 'return').reduce((s, t) => s + Math.abs(t.totalCost), 0);
  ...
  // 6. KNJIŽNI IZPISEK
  console.log('\n6. KNJIZNI IZPISEK:');
  const cashSales = paidOrders.filter(o => o.paymentMethod === 'gotovina').reduce((s, o) => s + o.total, 0);
  const cardSales = paidOrders.filter(o => o.paymentMethod === 'kartica').reduce((s, o) => s + o.total, 0);
  console.log(`  DEBIT:`);
