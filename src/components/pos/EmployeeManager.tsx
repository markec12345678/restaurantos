...
export function EmployeeManager() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Employee created'); queryClient.invalidateQueries({ queryKey: ['employees'] }); setDialogOpen(false) },
  })
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Employee updated'); queryClient.invalidateQueries({ queryKey: ['employees'] }); setDialogOpen(false); setEditingEmployee(null) },
  })
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Employee deleted'); queryClient.invalidateQueries({ queryKey: ['employees'] }) },
  })
  ...
  const createShiftMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Shift scheduled'); queryClient.invalidateQueries({ queryKey: ['shifts'] }); setShiftDialogOpen(false) },
  })
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage staff and shift scheduling</p>
        </div>
            ...
            <Card key={emp.id as string} className={`hover:shadow-md transition-shadow ${emp.status === 'inactive' ? 'opacity-60' : ''}`}>
              <CardContent className="p-4 space-y-3">
                <div className="flex items-start justify-between">
                  ...
                  <Badge className={roleColors[String(emp.role)] || ''}>{String(emp.role)}</Badge>
                </div>

                <div className="text-sm space-y-1">
                  {emp.phone && <p className="text-muted-foreground">📞 {String(emp.phone)}</p>}
                  <p className="text-muted-foreground">📅 Hired: {emp.hireDate ? format(new Date(emp.hireDate as string), 'MMM dd, yyyy') : 'N/A'}</p>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-xs text-muted-foreground">{String(emp.status)}</span>
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {shifts.slice(0, 12).map((shift: Record<string, unknown>) => (
              <div key={shift.id as string} className="flex items-center justify-between p-3 rounded-lg border bg-card">
                <div>
                  <p className="text-sm font-medium">{String((shift.employee as Record<string, unknown>)?.name || 'Unknown')}</p>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div><Label>Hire Date</Label><Input type="date" value={formData.hireDate} onChange={(e) => setFormData({ ...formData, hireDate: e.target.value })} /></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button>
            <Button onClick={handleSubmit} disabled={!formData.name || !formData.email}>{editingEmployee ? 'Update' : 'Create'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={shiftDialogOpen} onOpenChange={setShiftDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div><Label>Date</Label><Input type="date" value={shiftForm.date} onChange={(e) => setShiftForm({ ...shiftForm, date: e.target.value })} /></div>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => createShiftMutation.mutate(shiftForm)} disabled={!shiftForm.employeeId || !shiftForm.date}>Schedule</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
