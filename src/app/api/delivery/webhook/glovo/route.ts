...
export async function POST(req: Request) {
  try {
    ...
    const existing = await findExistingGlovoOrder(glovoIntegration.id, glovoOrder.order_id, webhookLocationId)
    if (existing) {
    ...
    const txResult = await db.$transaction(async (tx) => {
      ...
      const freshDup = await findExistingGlovoOrder(glovoIntegration.id, glovoOrder.order_id, webhookLocationId, tx)
      if (freshDup) {
