...
export const ReceiptTotalsSection = memo(function ReceiptTotalsSection({
  ...
  return (
    <div className="space-y-0.5">
      {/* P2-UX FIX (decimalna vejica): formatNumberSl — slovenska vejica na računu */}
      ...
      <div className="flex justify-between font-medium">
        ...
        <span>{formatNumberSl(receipt.totalVat)} €</span>
      </div>

      {receipt.discount > 0 && (
        <div className="flex justify-between text-emerald-600">
          <span>Popust:</span>
          <span>-{formatNumberSl(receipt.discount)} €</span>
        </div>
      )}
      ...
      <div className="flex justify-between text-sm font-bold">
        ...
        <span>{formatNumberSl(receipt.total)} €</span>
      </div>
      ...
      {receipt.tip > 0 && (
        <>
          <div className="flex justify-between">
            <span>Napitnina:</span>
            <span>{formatNumberSl(receipt.tip)} €</span>
          </div>
          <div className="flex justify-between font-bold">
            <span>SKUPAJ Z NAPITNINO:</span>
            <span>{formatNumberSl(receipt.totalWithTip)} €</span>
          </div>
        </>
      )}
    </div>
