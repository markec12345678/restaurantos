        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide">
          {suggestions.slice(0, 3).map((sug, i) => (
            <button
              ...
              aria-label={`${sug.name} ${formatEUR(sug.price * 1.22)}. ${sug.reason}`}
            >
              ...
              <p className={`text-xs font-bold mt-1 ${isDark ? 'text-amber-400' : 'text-amber-700'}`}>+{formatEUR(sug.price * 1.22)}</p>
            </button>
          ))}
        </div>
      </div>
