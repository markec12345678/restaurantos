        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {selectedItem.category?.name || ''} · {formatEUR(selectedItem.price)}
            </p>
          </div>
          ...
          <CardContent className="p-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
              <div>
                ...
                <p className="text-lg font-bold text-red-600">{formatEUR(selectedTotalCost)}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Prodajna cena</p>
                <p className="text-lg font-bold">{formatEUR(selectedItem.price)}</p>
              </div>
              <div>
                ...
                <p className={`text-lg font-bold ${marginPct >= 60 ? 'text-emerald-600' : marginPct >= 40 ? 'text-amber-600' : 'text-red-600'}`}>
                  {formatEUR(selectedItem.price - selectedTotalCost)}
                </p>
              </div>
              <div>
        ...
        {(Array.isArray(selectedRecipes) ? selectedRecipes : []).length > 0 ? (
          <div className="space-y-2">
              ...
              <div key={recipe.id} className="flex items-center justify-between p-3 rounded-lg border hover:shadow-sm transition-shadow">
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <p className="font-semibold text-sm">{formatEUR(recipe.costPerServing)}</p>
            ...
            <div className="flex justify-between p-3 rounded-lg bg-muted/50 font-semibold text-sm">
              ...
              <span>{formatEUR(selectedTotalCost)}</span>
            </div>
          </div>
        ) : (
