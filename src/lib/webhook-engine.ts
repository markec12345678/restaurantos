...
export async function triggerWebhook(
  ...
  for (const webhook of matchingWebhooks) {
    ...
    deliverAndLog(webhook, event, data, restaurantInfo).catch(err => {
      console.error(`[WebhookEngine] Napaka pri dostavi webhook ${webhook.id}:`, err)
    })
  }
}
...
async function deliverAndLog(
  ...
  if (payloadStr.length > MAX_PAYLOAD_SIZE) {
    console.warn(`[WebhookEngine] Payload prevelik (${payloadStr.length} bajtov) za webhook ${webhook.id}`)
    return
  }
