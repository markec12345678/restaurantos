...
export async function POST() {
  try {
    ...
    const menuItems = await Promise.all([
      // Appetizers
      db.menuItem.create({ data: { name: 'Caesar Salad', description: 'Crisp romaine with parmesan and croutons', price: 9.99, categoryId: appetizers.id, sortOrder: 0 } }),
      db.menuItem.create({ data: { name: 'Bruschetta', description: 'Toasted bread with fresh tomato and basil', price: 8.49, categoryId: appetizers.id, sortOrder: 1 } }),
      db.menuItem.create({ data: { name: 'Spring Rolls', description: 'Crispy vegetable spring rolls with dip', price: 7.99, categoryId: appetizers.id, sortOrder: 2 } }),
      db.menuItem.create({ data: { name: 'Soup of the Day', description: 'Freshly made daily soup', price: 6.99, categoryId: appetizers.id, sortOrder: 3 } }),
      // Main Course
      db.menuItem.create({ data: { name: 'Grilled Salmon', description: 'Atlantic salmon with lemon butter sauce', price: 24.99, categoryId: mainCourse.id, sortOrder: 0 } }),
      db.menuItem.create({ data: { name: 'Ribeye Steak', description: '12oz ribeye cooked to your preference', price: 32.99, categoryId: mainCourse.id, sortOrder: 1 } }),
      db.menuItem.create({ data: { name: 'Chicken Parmesan', description: 'Breaded chicken with marinara and mozzarella', price: 18.99, categoryId: mainCourse.id, sortOrder: 2 } }),
      db.menuItem.create({ data: { name: 'Lamb Chops', description: 'Herb-crusted lamb chops with rosemary', price: 28.99, categoryId: mainCourse.id, sortOrder: 3 } }),
      // Pasta
      db.menuItem.create({ data: { name: 'Spaghetti Carbonara', description: 'Classic carbonara with pancetta and egg', price: 16.99, categoryId: pasta.id, sortOrder: 0 } }),
      db.menuItem.create({ data: { name: 'Fettuccine Alfredo', description: 'Creamy alfredo sauce with parmesan', price: 15.99, categoryId: pasta.id, sortOrder: 1 } }),
      db.menuItem.create({ data: { name: 'Penne Arrabbiata', description: 'Spicy tomato sauce with garlic and chili', price: 14.49, categoryId: pasta.id, sortOrder: 2 } }),
      db.menuItem.create({ data: { name: 'Lasagna', description: 'Layers of pasta, meat sauce, and cheese', price: 17.99, categoryId: pasta.id, sortOrder: 3 } }),
      // Pizza
      db.menuItem.create({ data: { name: 'Margherita', description: 'Fresh mozzarella, tomato, and basil', price: 14.99, categoryId: pizza.id, sortOrder: 0 } }),
      db.menuItem.create({ data: { name: 'Pepperoni', description: 'Classic pepperoni with mozzarella', price: 16.99, categoryId: pizza.id, sortOrder: 1 } }),
      db.menuItem.create({ data: { name: 'BBQ Chicken', description: 'BBQ sauce, chicken, and red onion', price: 18.49, categoryId: pizza.id, sortOrder: 2 } }),
      db.menuItem.create({ data: { name: 'Vegetarian', description: 'Bell peppers, mushrooms, olives, and onion', price: 15.99, categoryId: pizza.id, sortOrder: 3 } }),
      // Burgers
      db.menuItem.create({ data: { name: 'Classic Burger', description: 'Beef patty with lettuce, tomato, and onion', price: 13.99, categoryId: burgers.id, sortOrder: 0 } }),
      db.menuItem.create({ data: { name: 'Bacon Cheeseburger', description: 'Beef patty with bacon and cheddar', price: 16.49, categoryId: burgers.id, sortOrder: 1 } }),
      db.menuItem.create({ data: { name: 'Mushroom Swiss', description: 'Beef patty with sautéed mushrooms and Swiss', price: 15.99, categoryId: burgers.id, sortOrder: 2 } }),
      db.menuItem.create({ data: { name: 'Veggie Burger', description: 'Plant-based patty with avocado', price: 14.49, categoryId: burgers.id, sortOrder: 3 } }),
      // Desserts
      db.menuItem.create({ data: { name: 'Tiramisu', description: 'Classic Italian coffee dessert', price: 9.99, categoryId: desserts.id, sortOrder: 0 } }),
      db.menuItem.create({ data: { name: 'Chocolate Lava Cake', description: 'Warm chocolate cake with molten center', price: 10.99, categoryId: desserts.id, sortOrder: 1 } }),
      db.menuItem.create({ data: { name: 'Cheesecake', description: 'New York style cheesecake', price: 8.99, categoryId: desserts.id, sortOrder: 2 } }),
      db.menuItem.create({ data: { name: 'Crème Brûlée', description: 'Vanilla custard with caramelized sugar', price: 9.49, categoryId: desserts.id, sortOrder: 3 } }),
      // Beverages
      db.menuItem.create({ data: { name: 'Fresh Lemonade', description: 'House-made lemonade', price: 4.99, categoryId: beverages.id, sortOrder: 0 } }),
      db.menuItem.create({ data: { name: 'Iced Coffee', description: 'Cold brew with cream', price: 5.49, categoryId: beverages.id, sortOrder: 1 } }),
      db.menuItem.create({ data: { name: 'Sparkling Water', description: 'San Pellegrino', price: 3.49, categoryId: beverages.id, sortOrder: 2 } }),
      db.menuItem.create({ data: { name: 'Mango Smoothie', description: 'Fresh mango and yogurt blend', price: 6.99, categoryId: beverages.id, sortOrder: 3 } }),
      db.menuItem.create({ data: { name: 'Craft Beer', description: 'Local IPA on tap', price: 7.99, categoryId: beverages.id, sortOrder: 4 } }),
      db.menuItem.create({ data: { name: 'House Wine', description: 'Red or white, by the glass', price: 9.99, categoryId: beverages.id, sortOrder: 5 } }),
      // Sides
      db.menuItem.create({ data: { name: 'French Fries', description: 'Crispy golden fries', price: 5.49, categoryId: sides.id, sortOrder: 0 } }),
      db.menuItem.create({ data: { name: 'Garlic Bread', description: 'Toasted with garlic butter', price: 4.99, categoryId: sides.id, sortOrder: 1 } }),
      db.menuItem.create({ data: { name: 'Coleslaw', description: 'Creamy coleslaw', price: 3.99, categoryId: sides.id, sortOrder: 2 } }),
      db.menuItem.create({ data: { name: 'Onion Rings', description: 'Beer-battered onion rings', price: 5.99, categoryId: sides.id, sortOrder: 3 } }),
    ])
