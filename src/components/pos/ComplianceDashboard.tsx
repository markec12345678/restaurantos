...
export function ComplianceDashboard() {
  ...
  const loadCompliance = async () => {
    try {
      ...
      // Preveri HACCP vnose
      const haccpRes = await fetch('/api/haccp')
      ...
      const menuItemsWithAllergens = (menuData || []).filter((i: any) => i.allergens && i.allergens.length > 0)
      const menuItemsWithoutAllergens = (menuData || []).filter((i: any) => !i.allergens || i.allergens.length === 0)
