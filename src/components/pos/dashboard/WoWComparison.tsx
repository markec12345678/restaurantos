            ...
            <div className="flex items-center justify-between mb-1">
              ...
              {(wow?.changes?.revenue || 0) >= 0 ? (
                <Badge className="bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400 text-[10px]">
                  <ArrowUpRight className="h-3 w-3 mr-0.5" />+{safeToFixed(wow?.changes?.revenue || 0, 1)}%
                </Badge>
              ) : (
                <Badge className="bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400 text-[10px]">
                  <ArrowDownRight className="h-3 w-3 mr-0.5" />{safeToFixed(wow?.changes?.revenue || 0, 1)}%
                </Badge>
              )}
            </div>
            <p className="text-lg font-bold">{formatEUR(wow?.thisWeek?.revenue || 0)}</p>
            <p className="text-[10px] text-muted-foreground">Prejšnji teden: {formatEUR(wow?.lastWeek?.revenue || 0)}</p>
          </div>
          {/* Naročila WoW */}
          <div className="p-3 rounded-lg bg-muted/50">
            <div className="flex items-center justify-between mb-1">
              ...
              {(wow?.changes?.orders || 0) >= 0 ? (
                <Badge className="bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400 text-[10px]">
                  <ArrowUpRight className="h-3 w-3 mr-0.5" />+{safeToFixed(wow?.changes?.orders || 0, 1)}%
                </Badge>
              ) : (
                <Badge className="bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400 text-[10px]">
                  <ArrowDownRight className="h-3 w-3 mr-0.5" />{safeToFixed(wow?.changes?.orders || 0, 1)}%
                </Badge>
              )}
            </div>
          ...
          <div className="p-3 rounded-lg bg-muted/50">
            <div className="flex items-center justify-between mb-1">
              ...
              {(wow?.changes?.avgOrder || 0) >= 0 ? (
                <Badge className="bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400 text-[10px]">
                  <ArrowUpRight className="h-3 w-3 mr-0.5" />+{safeToFixed(wow?.changes?.avgOrder || 0, 1)}%
                </Badge>
              ) : (
                <Badge className="bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400 text-[10px]">
                  <ArrowDownRight className="h-3 w-3 mr-0.5" />{safeToFixed(wow?.changes?.avgOrder || 0, 1)}%
                </Badge>
              )}
            </div>
            <p className="text-lg font-bold">{formatEUR(wow?.thisWeek?.avgOrder || 0)}</p>
            <p className="text-[10px] text-muted-foreground">Prejšnji teden: {formatEUR(wow?.lastWeek?.avgOrder || 0)}</p>
          </div>
        </div>
        {/* Dnevni WoW diagram */}
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={wowChartData} barGap={4}>
                ...
                <Tooltip formatter={(value, name) => [`${formatEUR(Number(value ?? 0))}`, String(name)]} contentStyle={{ borderRadius: '8px', border: '1px solid var(--border)' }} />
