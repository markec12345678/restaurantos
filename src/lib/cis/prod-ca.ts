// ============================================
// FINA PRODUKCIJSKA CA VERIGA — javni certifikati za CIS PRODUKCIJO
// ============================================
// Vir: AIA razširitve produkcijske verige (uradni FINA viri):
//   - Fina Root CA:        http://rdc.fina.hr/Root/FinaRootCA.cer
//   - Fina RDC CA 2020:    http://rdc.fina.hr/RDC2020/FinaRDCCA2020.cer
// Prod CIS strežnik (cis.porezna-uprava.hr:8449) NE pošilja intermediate-a v
// TLS handshake (openssl verify code 21 "unable to verify the first
// certificate") — klient mora podati verigo sam. Javni certifikati — ni
// skrivnosti. Veriga je bila z openssl verify -CAfile verificirana proti
// produkcijskemu listu (OK).

export const CIS_PROD_CA_PEM = [
  // Fina Root CA
  `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,
  // Fina RDC CA 2020
  `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,
].join('\n')

