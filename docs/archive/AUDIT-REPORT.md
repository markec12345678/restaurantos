...
- **#36** ✅ FIXED (PR #63) — `Shift` vs `StaffShift` prekrivanje ~80% — novo `getUnifiedShifts()` helper združi oba v skupen `UnifiedShift` format; `Shift` model @deprecated z migracijskim planom; `getShiftSourceStats()` za migracijski dashboard
...
### LOW
- **#40** ✅ FIXED (PR #66) — Prisma provider mismatch — `migration_lock.toml` je že popravljen (postgresql), ampak `.env.example` je imel SQLite path (`file:./db/custom.db`) kar je neveljavno za postgresql schema. Sedaj: pravilna dokumentacija (PGlite za dev, external postgresql za prod), nov `validateDatabaseConfig()` helper z `detectProvider` + `maskDatabaseUrl`, nov admin endpoint `GET /api/system/db-health` + 18 testov
- **#41** ✅ FIXED (PR #68) — 0 enumov — 20+ status polj so prosto-besedilni String-i — novo centralni `src/lib/enums/` z 14 TS const objects (OrderStatus, OrderType, PaymentStatus, ShiftStatus, AccountType, SubscriptionPlan, FursEnvironment, itd.) + 14 type-guards (`isXxxStatus`) za runtime validacijo + `enumValues` helper + `getEnumStats()` dashboard; 39 testov zaklepi vrednosti
- **#42** ✅ FIXED (PR #67) — Dokumentacija trdi "22 IndexedDB trgovin" — dejansko 2 — dodana eksplicitna konstanta `INDEXEDDB_STORES = ['pendingOrders', 'pendingReceipts']` + `INDEXEDDB_STORE_COUNT = 2` v `src/lib/offline-furs/index.ts`; 4 testi zaklepijo dejansko število
...
- **#44** ✅ FIXED (PR #65) — 3 paralelni i18n sistemi (next-intl messages, src/lib/i18n, src/i18n/request.ts) — novo centralni `tTranslate(key, locale)` proxy z 3-stopenjskim fallback (nextIntl > legacy > key); `getI18nStats()` migracijski dashboard z števci + recommendations
