              ...
              <div key={zone.id} className="flex items-center justify-between p-3 rounded-xl border bg-muted/30">
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                    <span>Dostava: {formatEUR(zone.deliveryFee || 0)}</span>
                    <span>Min: {formatEUR(zone.minOrderAmount || 0)}</span>
                    {zone.freeDeliveryAbove != null && zone.freeDeliveryAbove > 0 && <span className="text-green-600">Brezplačno nad {formatEUR(zone.freeDeliveryAbove)}</span>}
