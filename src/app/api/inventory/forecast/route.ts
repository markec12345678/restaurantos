...
function holtWintersForecast(
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const error = data[i].quantity - forecastVal
  ...
  try {
    ...
    for (const item of inventoryItems) {
      ...
      for (const tx of transactions) {
        ...
        dailyMap.set(dateKey, (dailyMap.get(dateKey) || 0) + Math.abs(tx.quantity))
      }
      ...
      const { forecast, trend: hwTrend } = holtWintersForecast(dailyUsage)
      ...
      if (avgDailyUsage > 0) {
        daysUntilEmpty = Math.floor(item.quantity / avgDailyUsage)
      }
      ...
      const needsReorder = item.quantity <= reorderPoint || (daysUntilEmpty !== null && daysUntilEmpty <= 7)

      // Predlagana količina naročila
      // Uporabi economic order quantity (EOQ) poenostavljen model
      const leadTimeDays = 3 // Privzeto 3 dni za dobavo
      const reorderQty = needsReorder
        ? Math.ceil(forecast14d + safetyStock - item.quantity)
        : 0
      ...
      results.push({
        ...
        lastRestockQty: lastProcurement?.quantity || 0,
      })
    }
    ...
    console.error('Napaka pri napovedovanju zaloge:', error)
    return NextResponse.json({ error: 'Napaka pri napovedovanju zaloge' }, { status: 500 })
  }
}
