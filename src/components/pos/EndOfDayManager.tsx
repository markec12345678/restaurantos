...
import {
  CheckCircle2, XCircle, AlertTriangle, DollarSign, CreditCard,
  Banknote, Shield, Clock, FileText, TrendingUp, TrendingDown,
  Package, Users, Calendar, Receipt, ArrowRight, Lock,
...
export function EndOfDayManager() {
  ...
  const { data, isLoading, refetch } = useQuery<EODData>({
    queryKey: ['end-of-day'],
    queryFn: async () => {
  ...
  const eodChecks = [
    ...
    { label: 'Gotovina usklajena', done: false },
    { label: 'Dnevni kontrolni seznam zaključen', done: false },
  ]
  ...
  return (
    <div className="space-y-6 overflow-y-auto h-full p-1 custom-scrollbar">
      ...
      {/* ── Naročila ──────────────────────────────── */}
      <Card>
        <CardHeader className="p-3 cursor-pointer" onClick={() => toggleSection('orders')}>
          <div className="flex items-center justify-between">
      ...
      {/* ── Plačila ──────────────────────────────── */}
      <Card>
        <CardHeader className="p-3 cursor-pointer" onClick={() => toggleSection('payments')}>
          <div className="flex items-center justify-between">
      ...
      {/* ── DDV ──────────────────────────────── */}
      <Card>
        <CardHeader className="p-3 cursor-pointer" onClick={() => toggleSection('vat')}>
          <div className="flex items-center justify-between">
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea value={eodNotes} onChange={(e) => setEodNotes(e.target.value)} placeholder="Opombe za zaključek dneva..." rows={3} />
            </div>
          </div>
          <DialogFooter>
