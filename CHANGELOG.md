...
### 🔁 CI/E2E fix (run #543 — prva izvedba vodnika je odkrila tri zamotane hrošče)

CI na prvem v1.3.1 pushu je padel v jobu "E2E Security Tests (30)". Reprodukcija na identičnem okolju (svež PostgreSQL 16 + `db push` + e2e-seed + standalone build) je odkrila tri vzroke:

- **Fixed (test):** E-4 "dvojno zapiranje istega čeka" je naročal `mi-1` (artikel loc-1) na `table-2` (miza loc-2) — MEŠANI lokaciji. Nova MODEL A varovalka take naročilo PRAVILNO zavrne (400 "ni na voljo na tej lokaciji") — varnost deluje, testni podatki pa so bili zastareli. Popravljen na dosleden par loc-2 (`mi-4` + `table-2`)
- **Fixed (kaskada):** prijavo omejujeta DVE PLASTI, obe na 5/15 min: (1) middleware `API_RATE_LIMITS` (Edge, in-memory — `Preveč zahtev…`), ki se sproži PRVI, in (2) route-level `LOGIN_LIMIT` (`Preveč neuspešnih poskusov…`). Obe štejeta TUDI uspešne prijave — E2E zbirka naredi 5+ (4 datoteke × beforeAll + MODELA filiala prijava + playwright retriji); 6. prijava = 429 → MODELA beforeAll padel, observability beforeAll je tiho pustil `authToken=undefined` → OBS-1/OBS-2 padli na 401. Popravek: OBE plasti sta zdaj ENV-nastavljivi (`LOGIN_RATE_LIMIT_MAX`, privzeto 5 — produkcija NESPREMENJENA, CI dvigne na 30; enak vzorec kot obstoječi `API_RATE_LIMIT_MAX`) + observability beforeAll zdaj fail-fast (javen padec prijave namesto prikrite 401 kaskade)
- **Fixed (retry-robustno):** D-3 je trdo zahteval `-000001` — drži le za PRVI loc-2 račun na sveži bazi; playwright retriji (sproženi zaradi E-4 padca) pustijo dodatne loc-2 račune → lažni padec, ki bi prikril pravi vzrok. Zdaj pričakovana številka = `000001 + testInfo.retry` (asercija ostaja STROGA)
- **Fixed (uskladitev verzij, RELEASE_PROCESS mesto #7):** `/api/health` je poročal `1.0.13` (hardcodiran fallback), package.json pa 1.3.1 — APP_VERSION se zdaj inlined iz package.json ob buildu (`next.config.ts env`). README/SECURITY usklajena na v1.3.1
