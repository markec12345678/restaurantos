...
[![Version](https://img.shields.io/badge/version-1.26.0-86702b?style=flat-square)](https://github.com/markec12345678/restaurantos/releases)
...
[![Tests](https://img.shields.io/badge/tests-3850%20unit%20%2B%209%20integracija%20%2B%20210%20E2E-3c7a50?style=flat-square)](tests/)
[![Audit](https://img.shields.io/badge/razvoj-112%20QA%20rund%20complete-426990?style=flat-square)](docs/FINAL-SUMMARY.md)
...
> Pilot-ready POS sistem za restavracije z dvojnim fiskalnim stikalom **FURS (SI) + FINA (HR)**, offline delovanjem, AI napovedmi in multi-tenant arhitekturo. **Security closure po rundi 108**: 24 rund hardeninga (R85–R108) zaključenih z dokazi in regresijskimi testi — tenant scope vali (dashboard, delivery-tracking, reports/*, wallet), M2 fail-open razred izkoreninjen, per-location ordering tokene z revokacijo, delivery webhook envelope, eradikacija vseh anonimnih globalnih fallbackov (P0-C3B kanon), 429 rate-limit kanon na 100 % API površine, dvostopenjska prijava (employeeId binding + javen employees grid + Toast/Square UI + admin binding naprave) in Table.status='reserved' DB flip na rezervacijskem lifecycle-u, deploy forenzika (health → { version, commit }), WebAuthn/passkey device attestation fundacija z prijavno integracijo ("Prijava s ključem naprave" na vezani napravi), koncurenčni WRITE tokovi (rezervacije/HR/darilne kartice/qr-pay/blagajna/nabavno naročilo — TOCTOU razred iz R100) z atomarnimi CAS/Serializable/advisory-lock patterni + enoten zalogovni pisalni kanon (odpis/absolutna prilagoditev/restock/ročna nastavitev na skupnem per-item ključu ključavnice) + CAS state machine na HR odobritvah dopustov + per-account ključavnica na ročnih prilagoditvah zvestobnih točk + pisalni kanon naročil (add-items: totals iz TX-FRESH seznama + razknjižba zaloge v isti transakciji) in miznih operacij (transfer/merge: sorted per-table ključavnice + CAS preklici) + CAS na ročnem outbox retry-ju; full-suite E2E (210 testov / 15 specov, vključno z virtual authenticator FIDO2 ceremony) zelen v CI na realnem PostgreSQL. Glej [Security Policy](SECURITY.md), [Final Summary](docs/FINAL-SUMMARY.md) in [Production Readiness](docs/PRODUCTION-READINESS-CHECKLIST.md).

### 🔒 Hardening v v1.26.0 (QA runde 85–112 — tenant scope kanon + ordering tokeni + 429 kanon + dvostopenjska prijava + deploy forenzika + WebAuthn fundacija + E2E v CI + Prisma query validacija + WebAuthn prijava + atomarni counter guard + multi-origin allowlist + koncurenčni WRITE tokovi + HR/darilne kartice concurrency + qr-pay/blagajna concurrency + nabavni prevzem concurrency + zalogovni pisalni kanon + time-off CAS/loyalty točkovni kanon + naročila/mize pisalni kanon + outbox retry CAS + čekovni pisalni kanon/void recalc kanon/plačilni lock pariteta + EOD CAS/Z-poročilo finalize kanon + KOT status guard/FURS verify claim/loyalty dnevna bonus idempotenca + order/KDS write-path CAS + delivery webhook idempotenca + happy-hour lifecycle lock + rate-limit kvote/input validacija)
...
| **R103** | HR (time-entries/staff-shifts) + darilne kartice: concurrency + error kontrakt | **10 najdenih lukenj zaprtih**: (T1, HIGH) **POST /api/time-entries TOCTOU double clock-in** — open-entry probe + create ločena → dva sočasna clock-in = dva aktivna vnosa = dvojna plača → `$transaction(Serializable)` (tx-fresh re-read zaposlenega + probe + create), P2034 → 409; (T2, MEDIUM) **cross-tenant leak** — 409 openEntry odgovor (existingEntry.id + clockIn!) pred employee scope validacijo → employee+scope NAJPREJ; (T3) PUT time-entries → scoped `updateMany` (P2025 → 404) + **T4** odobren vnos immutable (400 "plačilna integriteta"); (G1, HIGH) **PUT /api/gift-cards/[id] TOCTOU load-cap** — stale cap check + nepogojen increment → dva sočasna naloga presežeta initialBalance (tiskanje denarja) → pogojni `updateMany { balance: { lte: maxBalance − diff } }`; (G2) suspended/potekla re-validacija ZNOTRAJ tx; (G3) auto-ledger iz appliedDelta + post-op balanceAfter (prej stale compare = napačen znesek/smer v fiskalni zgodovini); (G5) DELETE → scoped `deleteMany` + P2003 → 409; (S1, HIGH) **POST /api/staff-shifts TOCTOU double-booking** → Serializable tx; (S2) PATCH → CAS `updateMany { status: existing.status }` (confirmedAt brez stale race) + (S3) DELETE scoped; (G8/error kontrakt) strukturirani tx throw-i (400/404/409) prej 500 "[object Object]" → canonical `src/lib/structured-error.ts` (R102 vzorec povzignen v lib); +42 testov (3542/3542, 197) |

| **R104** | QR-pay confirm (javna denarna pot) + blagajna shift lifecycle: concurrency & idempotency (TOCTOU razred iz R100–R103) | **5 najdenih lukenj zaprtih**: (Q1, HIGH) **POST /api/qr-pay/confirm TOCTOU double-charge** — celoten tok read-then-act BREZ transakcije (paymentStatus pre-check + idempotencyKey findFirst stale; create/update ×3 ločeni klici): ISTI token → P2002 → 500 čeprav je plačilo uspelo; RAZLIČNA tokena (re-init izda nov HMAC token, stari veljaven do TTL R82-D) → idempotencyKey različen → unique NE ščiti → **gost plača DVAKRAT** → FIX: `$transaction(Serializable)` + `pg_advisory_xact_lock(hashtext(checkId))` (kanon staff plačilne poti create-payment.ts) + tx-fresh re-read + P2002 race-path → 200 obstoječe (nikoli 500) + P2034 → 409; (Q2, HIGH) **over-collection** — amount = celoten total + tip ne glede na paidSoFar → delno plačan ček zaračunan ŠE ENKRAT v celoti → FIX: amount = preostanek (`round2(total − paidSoFar)`), tip ločeno (Z-report/cash-close ne štejeta napitnine dvakrat); (Q3) idempotency kontrakt — isti-token replay → 200 z obstoječim paymentId (SKB pravilo), brez dupliciranega audit loga; (C1, HIGH) **PUT /api/cash-register/[id] TOCTOU double-close** — status check znotraj transakcije ne ščiti pod READ COMMITTED → drugi close prepiše Z-report agregat + duplicira postShiftCloseActions (audit + webhooki + Z-osnutek ×2) → pogojni `updateMany { status: 'open' }`, count 0 → SHIFT_ALREADY_CLOSED (R100 atomic CAS vzorec); (C2) **POST /api/cash-register TOCTOU double-open** — findFirst(open)-then-create → dve odprti izmeni na isti lokaciji → Serializable + P2034 → 409; + strukturirani error kontrakt (400/404/409 prej 500 "[object Object]"); NOV deterministični E2E infra (scripts/e2e-chunked.sh + e2e-resumable.sh — per-spec server restart + NODE_OPTIONS heap cap za 4 GB sandbox; critical-path ZOI smoke test self-seed — prej odvisen od cross-spec stanja, chunked fresh-seed mode ga je razkril); +19 testov (3561/3561, 198) |

| **R105** | Nabavno naročilo: prevzem blaga (Goods Receipt) concurrency & idempotency + PUT/PATCH CAS state machine (TOCTOU razred iz R100–R104) | **6 najdenih lukenj zaprtih**: (PO-1, HIGH) **TOCTOU double-receive** — PO+items stale read IZVEN tx v OBEH kopijah prevzemnega toka (POST receive + PUT/PATCH action=receive); read-modify-write `quantityReceived` → dva sočasna prevzema = dvojna zaloga + dve StockTransaction; PUT pot sploh brez `received` status guard-a → FIX: **EDEN skupni kanon `receivePurchaseOrderItems()`** — `$transaction(Serializable)` + `pg_advisory_xact_lock(hashtext(poId))` + tx-fresh re-read + validacija samo proti svežim podatkom; (PO-2, HIGH) **dvojna obveznost** — AP create brez pregleda + `apNumber @unique` count+1 števec → P2002 → 500 (R104 Q1 razred) → tx-fresh `findFirst { purchaseOrderId }` skip-if-exists + P2002/P2034 → 409 (nikoli 500); (PO-3) prevzem **cancelled** naročila → tiho povečanje zaloge → 400 (tx-fresh); (PO-4) neznan itemId v PUT poti → tihi `continue` = 200 brez vsakega pisanja (revizijska neskladja) → fail-closed 400; (PO-5, MEDIUM) **PUT/PATCH state machine last-write-wins** — stale `existing` + validacija + NEPOGOJEN `update({ where: { id } })` → dva sočasna prehoda (submitted→approved IN →cancelled) oba preživita → CAS `updateMany { id, status: existing.status }` count 0 → 409; (PO-6) error kontrakt — string-matching catch → `structuredErrorResponse` (canonical); +24 testov (3585/3585, 200) |

| **R106** | Zaloga: enoten pisalni kanon (odpis/absolutna prilagoditev/restock/ročna nastavitev) — concurrency & error kontrakt (TOCTOU razred iz R100–R105) | **3 najdene luknje zaprte, 4 pisalne poti pod ENIM per-item ključem ključavnice**: (INV-1, HIGH) **POST /api/inventory/adjust negativna zaloga** — odpisna pot: tx re-read + `delta = -min(qty, currentQty)` + NEPOGOJEN `decrement` → dva sočasna odpisa oba preživita cap proti istemu stale branju → **negativna zaloga** (batch PUT je imel P3 atomarni guard, POST NI; prodaja nezalogovanega blaga + FURS neusklajenost); absolutna pot ("nastavi na 100" z 50): `delta = target − currentQty` → dva sočasna adjusta = dvojna delta (LOST UPDATE) → FIX: **skupni kanon `adjustInventoryItemStock()`** (`src/app/api/inventory/_helpers/stock-mutations.ts`) — `$transaction(Serializable)` + `pg_advisory_xact_lock(hashtext('inv-stock:' + itemId))` + tx-fresh scoped re-read + validacija samo proti svežim podatkom + atomarni `updateMany { quantity: { gte } }` decrement (dvorno varovalo — negativna zaloga nemogoča); (INV-2, HIGH) **PUT/PATCH /api/inventory/[id] lost update** — `existing` stale read IZVEN tx + `diff` iz stale + NEPOGOJEN absolute `update({ where: { id } })` → sočasna prodaja (decrement) tiho PREPIŠANA + duplirana StockTransaction vrstica (revizijska neskladja) → FIX: **kanon `setInventoryItemQuantity()`** — lock + Serializable + tx-fresh re-read, `diff` iz sveže zaloge (fresh === target → BREZ StockTransaction, prej lažni audit zapis); (INV-3, MEDIUM) **POST /api/inventory/restock** — tx telo na raw `where: { id }` brez scope re-checka (izbris med stopnicama → P2025 → 500) + brez ključavnice (audit prepleti) → FIX: **kanon `restockInventoryItem()`** — tx-fresh scoped re-read (strukturirana 404) + SKUPNI lock ključ z adjust/PUT/PATCH → StockTransaction veriga (previousQty → newQty) brezvsnežna čez VSE pisalne poti; (INV-4) error kontrakt — `throw new Error('Artikel ni najden')` → 500 + string-matching → strukturirani `{ error, status }` throw-i + `structuredErrorResponse` + P2002/P2034 → 409 (nikoli 500); audit log z TX-FRESH vrednostmi (prej stale); +24 testov (3609/3609, 200: A kanon 12 / B PUT-PATCH pariteta 4 / C restock+skupni ključ 5 / D fs-pini 3) |

| **R107** | HR odobritve dopustov (CAS state machine) + ročne prilagoditve zvestobnih točk (per-account kanon) — concurrency & error kontrakt (TOCTOU razred iz R100–R106) | **4 najdene luknje zaprte**: (TO-1, HIGH) **POST /api/time-off/[id]/approve|reject state-machine bypass** — NEPOGOJEN `update({ where: { id } })` za stale scoped read-om → (a) **rejected → approved bypass** ( rejected/cancelled prošnja aktivirana brez guard-a), (b) **approve ∥ reject race** = last-writer-wins (dva managerja, oba stale read, reviewedAt dvakrat povožen), (c) **replay overwrite** reviewedAt, (d) **reviewedBy NIKOLI zapisan** (revizijska vrzel) → FIX: **CAS kanon `reviewTimeOffRequest()`** (`src/app/api/time-off/[id]/_helpers.ts`) — atomarni `updateMany { id, status: 'pending', employee: { locationId } }` (scope ZNOTRAJ where → scope drift nemogoč) + count 0 → tx-fresh re-read razloči 404 (ne obstaja/izven scope) · 200 replay (isti izid, BREZ overwrite-a revizijskih polj) · 409 konflikt; reviewedBy + reviewedAt v istem zapisu; (LO-1, HIGH) **PUT /api/loyalty/[id] lost update / dvojna delta** (R106 INV-1b dvojček) — `existing` stale read IZVEN tx + `diff = newPoints − stale` → dva sočasna "nastavi na 100" (iz 50) = oba +50 → končno 150 → FIX: **kanon `updateLoyaltyAccountWithLock()`** (`src/app/api/loyalty/_helpers/points-mutations.ts`) — `$transaction(Serializable)` + `pg_advisory_xact_lock(hashtext('loyalty-points:' + accountId))` + tx-fresh scoped re-read + `diff` iz svežega stanja + atomarni `updateMany { pointsBalance: { gte } }` decrement guard (unovčenje nikoli pod 0) + diff === 0 → brez LoyaltyTransaction (prej lažni audit); (LO-2/LO-3, MEDIUM) audit zapis + tier upgrade s TX-FRESH vrednostmi (prej stale diff/tier); (LO-4, MEDIUM) **POST /api/loyalty duplikat race** — check-then-act pregled → DB delni unique indeks strese P2002 → prej 500, sedaj 409 (PO-2 pariteta); error kontrakt (string-matching → structuredErrorResponse) čez VSE tri rute; +28 testov (3637/3637, 201: A time-off CAS 10 / B loyalty kanon 12 / C POST P2002 2 / D fs-pini 4) |
| **R108** | Naročila (add-items/transfer) + mizne operacije (transfer/merge) pisalni kanon + outbox retry CAS — concurrency & error kontrakt (TOCTOU razred iz R100–R107) | **6 najdenih lukenj zaprtih**: (OR-1, HIGH) **POST /api/orders/[id]/add-items lost update na totals** — recalc iz tx-fresh seznama, ampak BREZ izolacije/ključavnice + NEPOGOJEN `order.update({ totals })` → dva sočasna add-items oba prebereta isti seznam OrderItems, vsak doda svoje in zadnji prepiše totals z NSVEŽIM seznamom (brez tujih artiklov) → **total prenizek = PODRAČUNAVANJE** (revenue leak); (OR-1b, HIGH) status TOCTOU — stale check izven tx je edina zaščita → artikli na completed/cancelled naročilu → FIX: **kanon `addItemsToOrder()`** (`src/app/api/orders/[id]/_helpers/order-mutations.ts`) — `$transaction(Serializable)` + `pg_advisory_xact_lock(hashtext('order-write:' + orderId))` + tx-fresh scoped re-read + status CAS proti svežim podatkom + totals iz TX-FRESH seznama; (OR-1c, HIGH) **razknjižba zaloge v LOČENI transakciji** PO commit-u naročila → crash med njima = artikli brez odbitka zaloge (FURS/fizična neskladja) → FIX: NOV `deductStockForItemsInTx()` (`src/lib/stock-deduction/deduct-added.ts`) — odbitki v ISTI Serializable tx kot pisanje naročila; (OR-2, HIGH) **POST /api/orders/[id]/transfer** — NEPOGOJEN `update({ tableId })` brez tx-fresh re-reada/status preverbe → completed naročilo prenešeno, mizna stanja prepišana nad sočasen merge → FIX: **kanon `transferOrderToTable()`** — lock graf order→table (enosmeren = deadlock-varen) + tx-fresh re-read + status CAS + mizna stanja pod ključavnicama; (OR-4, HIGH) **POST /api/tables/transfer** — stale seznam naročil IZVEN tx + NEPOGOJEN premik → plačano/preklicano naročilo prenešeno (denarna sled razcepljena) + source miza 'available' z odprtim naročilom → FIX: **kanon `transferTableOrders()`** (`src/app/api/tables/_helpers/table-ops.ts`) — ključavnici OBEH miz v SORTED vrstnem redu (A→B ∥ B→A deadlock nemogoč) + tx-fresh re-read miz IN naročil + scoped count za free; (OR-5, HIGH) **POST /api/tables/merge** — stale items/orders IZVEN tx → (a) artikli dodani med branjem in tx ostanejo na PREKLICANEM naročilu (orphaned revenue), (b) NEPOGOJEN cancel source naročila → **plačano naročilo preklicano** (denarna sled pretrgana), (c) recalc iz stale discount/tip → lost update na totals → FIX: **kanon `mergeTables()`** — sorted ključavnice + tx-fresh items + CAS cancel (`updateMany { status in aktivni, paymentStatus: 'unpaid' }` → count 0 → 409) + recalc iz TX-FRESH podatkov; (OR-6, MEDIUM) **POST /api/outbox/[id]/retry** — NEPOGOJEN reset na pending → sent/processing event ponovno dostavljen = **dupli FURS/SMS/webhook delivery** → CAS `updateMany { status in ['failed','dead_letter'] }` → count 0 → 409; error kontrakt (string-matching handleRouteError izbrisan → structuredErrorResponse + P2002/P2034 → 409) čez VSE štiri rute; +38 testov (3675/3675, 202) |
| **R109** | Čekovni pisalni kanon (PUT/DELETE) + void recalc kanon + plačilna mutacijska lock pariteta — concurrency & error kontrakt (TOCTOU razred iz R100–R108) | **CK-1..CK-7**: ček PUT/DELETE state machine CAS (`updateMany { status }` count-guard), void artikla na čeku → recalc order+check v ENEM Serializable tx pod ključavnico `order-write:{orderId}` (totals iz TX-FRESH seznamov + svež paymentStatus guard — void na plačanem čeku nemogoč tudi v race-u), plačilne mutacije (create-payment/refund/qr-pay) na ISTEM ključu → lock graf brez križanj; P2002/P2034 → 409; +40 testov |
| **R110** | EOD zaključek dneva CAS kanon (close-shift) + Z-poročilo finalize kanon — concurrency & error kontrakt (TOCTOU razred iz R100–R109) | **EOD-1 (HIGH)**: dvojni close blagajniške izmene — NEPOGOJEN update je povožil finančne agregate + dupliral EOD_COMPLETED audit + dvojni finalize sprožilec → CAS `updateMany { status: 'open' }`, count 0 → idempotentna veja brez duplikatov; **ZR-1 (HIGH)**: sočasni finalize FINALIZED fiskalnega Z-poročila → **kanon** `$transaction(Serializable)` + `pg_advisory_xact_lock(hashtext('z-report:{locationId}:{date}'))` + tx-fresh re-check + CAS `updateMany { status: { not: 'finalized' } }`; **ZR-2**: statistike računane iz TX-FRESH podatkov (stale stats nikoli fiskalizirani); P2034 → strukturiran 409; +23 testov |
| **R111** | KOT pisalni kanon (status guard + Serializable tx) + FURS overitveni claim kanon (dvojni EOR preprečen) + loyalty dnevna bonus idempotenca — concurrency & error kontrakt (TOCTOU razred iz R100–R110) | **KOT-1 (MEDIUM)**: POST /api/kot — order read izven tx + brez status preverbe → KOT za PREKLICANO naročilo + check-then-act okno → Serializable tx + tx-fresh scoped re-read + status guard + create pod istim snapshot-om + counter znotraj tx po guardih; **FURS-1 (HIGH)**: verify check-then-act brez zaklepa → NEPOGOJEN zunanji FURS klic → **DVA EOR-ja za isti račun** (fiskalna kršitev ZDDV-1) → CAS claim (`updateMany fiscalStatus 'verifying'`) PRED zunanjim klicem; loser → idempotenten 200 (že overjen) ali 409 (in-flight); stale claim reclaim po 2 min; release na vseh failure poteh; **LA-1 (HIGH)**: loyalty triggerBirthdayBonus/triggerWinback brez dedup-a → dvoklik ali admin∥cron batch = **DVOJNI bonus (točke ×2)** + duplirani SMS → advisory lock `loyalty-bonus:{id}:{type}:{date}` + Serializable tx + tx-fresh re-check → skip; SMS šele PO commitu; P2034 → skip (batch ne pade); +34 testov |
| **R112** | Order/KDS write-path CAS + delivery webhook idempotenca/tenant-scoped mapping + happy-hour lifecycle + rate-limit kvote + input validacija (TOCTOU razred iz R100–R111) | **ORD-1 (HIGH)**: PATCH fire NEPOGOJEN update → **oživljanje cancelled/completed/plačanih naročil v KDS** → CAS `updateMany { status in [pending,in-progress,ready], paymentStatus != paid }`, count 0 → 409; **ORD-2**: fire auto-KOT check-then-act → **DVA original KOT-a** → advisory lock `kot-fire:{orderId}` + tx-fresh dedup + counter v tx; **ORD-3 (MED-HIGH)**: KDS item_status — stale cancelled guard + NEPOGOJEN item update (prepir z void claimom: KDS tap PREPIŠE void!) + auto-promotion regresija completed→ready → Serializable tx + lock `order-write:{orderId}` + tx-fresh guardi + CAS na artiklu IN naročilu; **ORD-4 (HIGH)**: DELETE cancel NEPOGOJEN → **race s plačilom PREKLICA PLAČANO naročilo** (denar prejet, zaloga vrnjena) → Serializable tx + tx-fresh paymentStatus guard (paid → 409 storno pot) + CAS žig + stranski učinki šele po žigu, strukturiran { ok, reason } → 404/400/409; **ORD-5/6**: order-items status/notes pot CAS (`voided: false` + order ne-cancelled) + promotion CAS (izključno iz pending/in-progress); **WEBHOOK-1/2 (MED-HIGH)**: Bolt/Wolt/Glovo dedup check-then-act (customerName CONTAINS / integrationLog JSON-scan, log ŠELE po create) → **provider redelivery storm = DVE plačani naročili** → Serializable tx + advisory lock `delivery-webhook:{integrationId}:{externalOrderId}` + tx-fresh dedup re-check → idempotenten 200 z obstoječim naročilom; **WEBHOOK-3/4 (MED)**: menu mapping BREZ tenant scopea (cross-tenant artikel = napačna cena/DDV) + Bolt `menuItems[0]` fallback + wire-cena → scoped `category.menu.locationId` + neznana pozicija → 400, cena IZKLJUČNO iz baze; **WEBHOOK-5 (MED)**: delivery status — 3 nepovezana writerja last-writer-wins → regresija delivered→picked_up → ENA skupna mapa prehodov (`status-transitions.ts`) + tx-fresh CAS `updateMany` na obeh writerjih (voznik + ročna UI); **HH-1..4**: happy-hour POST brez overlap validacije (neomejeno prekrivanj) → advisory lock `happy-hour:{locationId}` + Serializable + interval/dan presek → 409; PATCH CAS toggle (P2025 → 404); percentage popust 0–100 refine; **RL-1/2**: wallet-webhook (edini javni write brez limita) → DELIVERY_WEBHOOK_LIMIT; finančni subset (payments/refund/wallet/checks/z-report/eod) → AUTHENTICATED_LIMIT takoj za requireAuth; **VAL-1/2 + SEC-1**: config numeriki bounds (rate 0–100, amount ≥ 0, avgPrepTime 0–480, NaN prepovedan → 400); tip max 500 + **totalWithTip server-recalc** (client vrednost ignorirana); scheduled-emails sprejema IZKLJUČNO CRON_SECRET (WS_BROADCAST_SECRET reuse izkoreninjen); +175 testov (3850/3850, 209) |

**Test napredek:** 2495 (R84) → 3242 (R94) → 3315 (R95) → 3347 (R96) → 3410 unit / 189 datotek (R97) → 3416 unit / 190 datotek (R98) → 3542 unit / 197 datotek (R103) → 3561 unit / 198 datotek (R104) → 3585 unit / 200 datotek (R105) → 3609 unit / 200 datotek (R106) → 3637 unit / 201 datotek (R107) → 3675 unit / 202 datotek (R108) → 3850 unit / **209 datotek (R112)** + 9/9 integracija + **210/210 E2E v CI**.
