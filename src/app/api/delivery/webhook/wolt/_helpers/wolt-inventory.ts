...
// bila odbita. To je finančna nepravilnost (Wolt dobi order, mi pa ne
        ...
        if (updated.count > 0) {
          ...
          await tx.stockTransaction.create({
            data: {
              ...
              quantity: 0,
              previousQty: toNum(currentInv.quantity),
              newQty: toNum(currentInv.quantity),
              costPerUnit: toNum(currentInv.costPerUnit),
              totalCost: 0,
          ...
          logger.error(
            providerLabel,
            `Nezadostna zaloga za "${menuItem.name}" pri Wolt order #${orderNumber}: potrebno ${deductQty.toFixed(2)}, na voljo ${toNum(currentInv.quantity).toFixed(2)}`,
          )
