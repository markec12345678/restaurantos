...
export function VendorScorecard() {
  ...
  const loadSuppliers = async () => {
    try {
      ...
      const scored: SupplierScore[] = (supData || []).map((sup: any) => {
        const supplierOrders = (poData || []).filter((po: any) => po.supplierId === sup.id)

        // Izračunaj metrike
        const onTimeOrders = supplierOrders.filter((po: any) => {
        ...
        const totalSpent = supplierOrders.reduce((s: number, po: any) => s + (po.total || 0), 0)
        const totalOrders = supplierOrders.length

        const avgDeliveryDays = supplierOrders.length > 0
          ? supplierOrders.reduce((s: number, po: any) => {
        ...
        // Simulirane metrike (v praksi iz zgodovinskih podatkov)
        const qualityRating = 70 + Math.floor(Math.random() * 25)
        const priceCompetitiveness = 60 + Math.floor(Math.random() * 30)
        const responsiveness = 65 + Math.floor(Math.random() * 30)
        const orderAccuracy = 75 + Math.floor(Math.random() * 20)
        ...
        return {
          ...
          lastOrderDate: supplierOrders.length > 0
            ? supplierOrders.sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0].createdAt
            : null,
          trend,
          recentIssues,
          tier,
      ...
      console.error('Error loading vendor scorecards:', err)
    } finally {
      setLoading(false)
    }
  }
