...
import { ChefHat, Minimize, Maximize, RefreshCw, Wifi, WifiOff, Grid3X3, List, Volume2, VolumeX, RotateCcw, BellRing, BarChart3 } from 'lucide-react'
...
interface KDSHeaderProps {
  ...
  onToggleSound: () => void
  // R133: metrike panel toggle (pariteta toggle gumbov zvok/fullscreen)
  showMetrics: boolean
...
export const KDSHeader = memo(function KDSHeader({
  ...
  return (
    // R133 mobilni fix (lekcija R132): flex-wrap — na ozkih zaslonih (390px)
    // desna grupa (postaje + akcije) pade v drugo vrstico namesto horizontalnega
    // scrolla; ime zaposlenega truncate (min-w-0 veriga).
    <div className="flex flex-wrap items-center justify-between gap-y-2 px-4 py-2 border-b bg-card shadow-sm">
      <div className="flex items-center gap-3 min-w-0">
        <ChefHat className="w-6 h-6 shrink-0 text-orange-500" />
        <h1 className="text-lg font-bold shrink-0">KDS</h1>
        <span className="text-sm text-muted-foreground truncate max-w-[10rem]">{employeeName}</span>
      ...
      <div className="flex flex-wrap items-center gap-2 min-w-0">
        ...
        {/* R133: metrike — stanjsko barvanje (emerald = panel odprt, pariteta
            zvoku) + aria-pressed; ghost ikonski gumb, h-11 na pointer-coarse */}
        <button onClick={onToggleMetrics}
          aria-pressed={showMetrics}
          aria-label="Metrike"
          title="Metrike kuhinje"
          className={cn(
            'w-9 h-9 flex items-center justify-center rounded-lg touch-manipulation min-h-[36px] pointer-coarse:h-11 pointer-coarse:w-11 transition-colors',
            showMetrics
              ? 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 ring-1 ring-emerald-500/40 hover:bg-emerald-500/25'
              : 'bg-secondary text-muted-foreground hover:bg-secondary/80'
          )}>
        ...
        {bumpedCount > 0 && (
          <button onClick={onRecall}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-xs font-bold hover:bg-emerald-700 touch-manipulation min-h-[36px] pointer-coarse:min-h-[44px] pointer-coarse:px-4 pointer-coarse:text-sm">
