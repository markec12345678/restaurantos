    ...
    <div>
      ...
      <div className="bg-amber-50 dark:bg-amber-900/20 rounded-lg p-2 text-xs space-y-1">
        ...
        <div className="flex justify-between">
          ...
          <span className={`font-bold ${cashChange > 0 ? 'text-emerald-700 dark:text-emerald-400' : cashReceived > 0 ? 'text-red-600' : 'text-emerald-700 dark:text-emerald-400'}`} aria-live="polite">€{cashChange.toFixed(2)}</span>
        </div>
      </div>
    </div>
  )
