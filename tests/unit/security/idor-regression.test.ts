...
describe('P0-C1: IDOR Regression Tests — Tenant Isolation', () => {
  ...
  describe('Payment [id] — order.locationId in WHERE clause', () => {
    it('Tenant A user: WHERE includes order.locationId', async () => {
      ...
      // FIX P0-C1: Payment scoping prek check.order.locationId (Payment nima direktnega order relacije)
      expect(callArg.where).toHaveProperty('check')
      expect(callArg.where.check).toHaveProperty('order')
      expect(callArg.where.check.order).toHaveProperty('locationId', 'loc-tenant-a')
    })
