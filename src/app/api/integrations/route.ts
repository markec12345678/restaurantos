...
export async function POST(req: Request) {
  ...
  // Fiksni ključ 'integrations-post' (NE iz pathname): en IP ne more fan-out
  // prek različnih virov — pathname-izpeljan ključ bi vsakemu viru dal svoje
  // vedro. GET (seznam) ostane brez vedra — cenen determinističen read.
  const rateCheck = await checkRateLimitAsync('integrations-post', getClientIp(req), AUTHENTICATED_LIMIT)
