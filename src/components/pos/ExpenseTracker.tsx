...
import {
  DollarSign, Plus, TrendingDown, TrendingUp, Receipt,
  Building, Truck, Zap, Droplets, ShieldCheck, Wrench,
  Package, CreditCard, Banknote, RefreshCw, ArrowUpRight,
} from 'lucide-react'
    ...
    <div className="space-y-6 overflow-y-auto h-full p-1 custom-scrollbar">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => refetch()}><RefreshCw className="h-3 w-3" /></Button>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold">€{stats?.count > 0 ? ((stats.totalExpenses || 0) / stats.count).toFixed(2) : '0.00'}</p>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent>
            <div className="space-y-3">
              {CATEGORIES.map(cat => {
                ...
                const pct = stats?.totalExpenses ? (data.total / stats.totalExpenses) * 100 : 0
                return (
                  <div key={cat.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] text-muted-foreground mt-0.5">{data.count} vnosov · {pct.toFixed(1)}%</p>
                    </div>
                  </div>
                )
              })}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Opis stroška" />
            </div>
            <div>
              <label className="text-sm font-medium mb-1 block">Znesek (€)</label>
              <Input type="number" step="0.01" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} placeholder="0.00" />
            </div>
            <div>
              <label className="text-sm font-medium mb-1 block">Dobavitelj</label>
              <Input value={form.vendor} onChange={(e) => setForm({ ...form, vendor: e.target.value })} placeholder="Ime dobavitelja" />
            </div>
            <div className="flex items-center gap-4">
              <div className="flex-1">
                <label className="text-sm font-medium mb-1 block">Način plačila</label>
              ...
              <div className="flex items-center gap-2 pt-5">
                ...
                <span className="text-sm">Ponavljajoč</span>
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowAddDialog(false)}>Prekliči</Button>
            <Button onClick={() => addMutation.mutate(form)} disabled={!form.description || !form.amount || addMutation.isPending}>Dodaj</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
